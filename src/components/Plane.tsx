...
export function Plane(props) {
  const group = useRef();
  const [planeRotation, setPlaneRotation] = useState([0, -90, 0]);
  const [velocity, setVelocity] = useState(0);
  const { nodes, materials, animations } = useGLTF(
    "../src/components/stylized_ww1_plane.glb"
  );
  ...
  const handleArrowKey = (e) => {
    if (e.key === "ArrowLeft") {
      ...
      setPlaneRotation([
        planeRotation[0],
        planeRotation[1] < -2 ? planeRotation[1] -= 0.1 : planeRotation[1] = 0,
        planeRotation[2],
      ]);
    } else if (e.key === "ArrowUp") {
      // Incrementa la velocidad hacia adelante
      setVelocity(velocity - 0.1);
    } else if (e.key === "ArrowDown") {
      // Incrementa la velocidad hacia atrás (puedes ajustar el valor)
      setVelocity(velocity + 0.1);
    }
  };

  useFrame(() => {
    // Interpola la rotación actual hacia la rotación objetivo de manera suave.
    ...
    forwardVector.applyEuler(group.current.rotation);

    // Actualiza la posición del objeto en base a la velocidad
    group.current.position.x += forwardVector.x * velocity;
    group.current.position.y += forwardVector.y * velocity;
    group.current.position.z += forwardVector.z * velocity;

    // Actualiza la posición de la cámara para seguir el objeto
    const distanceBehindObject = 2; // Ajusta esta distancia según tus necesidades
    const cameraPosition = group.current.position.clone().add(forwardVector.clone().multiplyScalar(distanceBehindObject));
  ...
  return (
    <>
      <PerspectiveCamera makeDefault position={[2, 1, 1]} fov={80} />
...
useGLTF.preload("../src/components/stylized_ww1_plane.glb");
